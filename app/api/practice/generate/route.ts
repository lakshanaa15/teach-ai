...
import { generatePersonalizedPracticeWithGemini } from '@/lib/gemini'
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!studentId) {
      // Default to first student in system if not explicitly set
    ...
    // Query student performance records from PostgreSQL
    ...
    // Check for Material Analysis knowledge graph
    ...
    if (!hasQuizSubmissions && !hasLearningGaps && !hasMastery && !body.forceGenerate) {
      return NextResponse.json({
        ...
        message: 'Not enough performance data for personalized practice. Try completing a diagnostic quiz first.',
      })
    }
    ...
    // If still empty (e.g. force generate), populate from topic
    if (weakConcepts.length === 0) {
      weakConcepts.push(topic)
    }

    const previousPracticeQuestions = Array.isArray(body.previousQuestionTexts)
      ? body.previousQuestionTexts
      : undefined

    const practiceSet = await generatePersonalizedPracticeWithGemini({
      topic,
      ...
      materialAnalysisContext: materialAnalysis
        ...
        : undefined,
      numberOfQuestions: body.numberOfQuestions ? Number(body.numberOfQuestions) : 5,
      previousPracticeQuestions,
    })
    ...
    console.error('[GENERATE PERSONALIZED PRACTICE ERROR]:', error)
    return NextResponse.json(
      {
        success: false,
        error:
          error instanceof Error
            ? error.message
            : 'Failed to generate personalized practice questions.',
      },
      { status: 500 },
    )
  }
